...
if (isset($_GET['view_old']) and $_GET['view_old'] == 1)
{}
else
{
	
	header('Location: '. str_replace("view_graph2.php","view_graph3.php",$_SERVER['REQUEST_URI']));
	exit;
}
...
echo "<thead>";
$header_first_row = "<tr><th class='fixed_col_header' style='z-index: 160;    opacity: 0.8; border-bottom: none;'><input type='checkbox' onchange='change_col(this)'></th>";
$header_second_row = "<tr><th class='fixed_col_header' style='z-index: 150;'><br><input type='checkbox' value='all_rows' name='id_checkbox_all' onchange='select_deselect_all(this.checked)'></th>";
foreach ($info['schema']['fields'] as $bq_field)
{
	...
	if (!isset($field['hidden']) and in_array($field['db_field'], $displayed_field))
	{
		...
		{
		    $header_first_row     = "{$header_first_row}<th class='fixed_col_header' style='z-index: 160;    opacity: 0.8; border-bottom: none;'><input type='checkbox' onchange='change_col(this)'></th>";
		    $header_second_row    = "{$header_second_row}<th class='fixed_col_header' style='z-index: 150;'><br>{$field['name']}</th>"; 
		}
	}
} 
...
if (!$export_excel_only)
{
    $header_first_row   = "{$header_first_row}<th></th></tr>";
    $header_second_row  = "{$header_second_row}<th class='no_csv'>Drill</th></tr>";
    echo $header_first_row;
    echo $header_second_row;
	echo "</thead><tbody>";
}
else
{
...
<script>
var rows 					= document.getElementsByClassName("sortable")[0].children[1].children;
var checkbox_header_row 	= document.getElementsByClassName("sortable")[0].children[0].children[0];
var main_header_row 		= document.getElementsByClassName("sortable")[0].children[0].children[1];
var footer_row				= document.getElementsByClassName("sortable")[0].children[2].children[0];
...
<script type="text/javascript">
      ...
      function drawChart(params) 
	  {
		...
		var header 	= table.tHead.children[1];
		...
		var t1 = ' ';
		var t2 = '\u25B4';
		for(i=2;i< header.childElementCount;i++)
		{
			var clean_col_header = header.children[i].textContent;
			clean_col_header = clean_col_header.replace(t1 + t2,'');
			colNameIndex[clean_col_header] = i;
		} 
		...
		{	// Create Dynamicaly
			...
			for (j=0; j< params.length + 1; j++) 
			{
				if (j == 0)
				{
					matrix[0] = [header.children[1].innerText]
				}
				else
				{
			...
			{
				if (all_rows || body.children[i].children[0].children[0].checked)
				{
	...
	//					'Timelag'
	  ...
	  function fix_col(element) 
	  {
	  	...
	  	{
	  		if (checkbox_header_row.children[j].firstElementChild.firstElementChild.checked)
	  		{
	  			sum_offsetWidth 	= sum_offsetWidth + rows[0].children[j].offsetWidth;
	  		}
	  	}
	   
	  	element.parentElement.parentElement.style.left = sum_offsetWidth;
	  	element.parentElement.parentElement.style.zIndex = 180;
	  	
	  	var header = main_header_row.children[cell_index];
	  	header.style.left = sum_offsetWidth; 
	  	header.style.zIndex = 170;
	  	...
	  	for (var j=cell_index+1; j < checkbox_header_row.childElementCount -1; j++)
	  	{
	  		if (checkbox_header_row.children[j].firstElementChild.firstElementChild.checked)
	  		{
	  			fix_col(checkbox_header_row.children[j].firstElementChild.firstElementChild);
	  			break;
	  		}
	  	} 
	  }
	  ...
	  function select_row(element)
	  {
	  	if (element.checked)
	  	{
	  		element.parentElement.parentElement.classList.add('selected_row');
	  	}
	  	else
	  	{
	  		element.parentElement.parentElement.classList.remove('selected_row');
	  	}
	  }
