...
try
{
	...
	$list_for_product = "SELECT list1, list2,list3,list4,list5
						FROM products
						WHERE id = {$product_id} and status = 1";
	$ret_array['sql'][0] = $list_for_product;
	$result = $conn->query($list_for_product);
	$lists_for_product=mysqli_fetch_array($result,MYSQLI_ASSOC);
	//print_r($lists_for_slrs);
	if (sizeof($lists_for_product) == 0)
	{
	    $ret_array['error'] = ": Product " . $product_id . " -  No Longer Active";
		throw new Exception();
	}
	
	$list_id = 0;
	$empty_list_number = "";
	foreach ($lists_for_product as  $list_number=>$curr_list_id)
	{
		if ($curr_list_id != "")
		{
			$check_list = "	SELECT *
	...
	if ($list_id == 0)
	{
		...
		$create_list_sql = "INSERT INTO `lists` (`list_type_id`, `behavior`, `product_id`, `title`, `status`, `client_id`, `creation_type`) VALUES ( {$list_type_id}, {$behavior}, {$product_id}, 'pr_{$product_id}_{$list_type_id}_{$black_white_list}', 1, 1, 1);";
		...
		$update_new_list_to_product = "UPDATE products SET {$empty_list_number}='{$list_id}' WHERE id={$product_id};";
		$ret_array['sql'][3] = $update_new_list_to_product;
		if ($conn->query($update_new_list_to_product) === TRUE) 
		{
			//$list_id = $conn->insert_id;
			//echo "New record created successfully. Last inserted ID is: " . $list_id;
		} 
		...
		{
		    $ret_array['error'] = "Error: " . $update_new_list_to_product . "<br>" . $conn->error;
			throw new Exception();
		}
	}
